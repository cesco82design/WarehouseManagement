<?php

define('PATH', 'http://'.$_SERVER['SERVER_NAME']);
//define('PATH', 'http://localhost/gestionale');
define('LAYOUT', __DIR__.'/asset/layout/');
define('HOME', __DIR__.'/');
define('UTENTI', PATH.'/applicazioni/utenti/');
define('MAGAZZINO', PATH.'/applicazioni/magazzino/');
define('CLIENTI', PATH.'/applicazioni/clienti/');
define('FORNITORI', PATH.'/applicazioni/fornitori/');
define('SCHEDE', PATH.'/applicazioni/schede_clienti/');
define('TESSERE', PATH.'/applicazioni/tessere/');
define('TRATTAMENTI', PATH.'/applicazioni/trattamenti/');
define('APP', PATH . '/applicazioni');
define('APPL', __DIR__ . '/applicazioni/');
define('CSS', PATH.'/asset/css/');
define('IMG', PATH.'/asset/img/');
define('JS', PATH.'/asset/js/');
define('CLASMOD', __DIR__.'/asset/moduli/');